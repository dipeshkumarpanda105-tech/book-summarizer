...
var Background = /** @class */ (function () {
    ...
    Background.prototype.update = function (w, h) {
        ...
        if (this.type === 1) {
            ...
            if (_color) {
                this.drawRect(this._rect, _color, w, h, _radius);
    ...
    Background.prototype.drawRect = function (bg, _color, w, h, _radius) {
        bg.clear();
        bg.beginFill(_color);
...
var Circle = /** @class */ (function (_super) {
    ...
    Circle.prototype.drawGraph = function () {
        ...
        var virdiam = Math.min(this.width, this.height);
        var radius = this._radius;
        if (radius > 0) {
            vf.utils.deprecation("0.8.x", "Recommends using width and height instead of radius in circle");
            virdiam = Math.max(radius * 2, virdiam);
        }
        this._virtualRadius = virdiam * 0.5;
        if (this._color !== undefined)
            graphics.beginFill(this._color, this._fillAlpha);
        graphics.arc(this._anchorX ? (0.5 - this._anchorX) * virdiam : this._virtualRadius, this._anchorY ? (0.5 - this._anchorY) * virdiam : this._virtualRadius, this._virtualRadius, this._startAngle, this._endAngle, this._anticlockwise);
        graphics.endFill();
    };
    return Circle;
}(GraphBase_1.GraphBase));
...
var Image = /** @class */ (function (_super) {
    ...
    Image.prototype.commitProperties = function () {
        ...
        if (this._oldFillMode !== this._fillMode) {
...
var Label = /** @class */ (function (_super) {
    ...
    Object.defineProperty(Label.prototype, "color", {
        ...
        set: function (value) {
            ...
            color = Utils_1.hexListToIntList(value, color);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.color = exports.updateFontStyle = void 0;
/** ===================== mask  ===================== */
...
function updateFontStyle(target, key, value) {
    if (target.setInputStyle) {
        ...
        if (["linearGradientStops", "linearGradientType"].indexOf(key) >= 0) {
            target[key] = value;
        }
        else {
...
__exportStar(__webpack_require__(/*! ./systemInfo */ "./src/utils/systemInfo.ts"), exports);
...
function audio() {
    ...
    try {
        if (audioElem &&
            !!audioElem.canPlayType &&
            audioElem.canPlayType('audio/ogg; codecs="vorbis"').replace(/^no$/, "")) {
            audio.isOGGsupported = true;
        }
    }
    catch (e) {
        console.error(e);
...
function deviceOrientation() {
    ...
    return new Promise(function (resolve, reject) {
        var w = window;
        if (w.DeviceOrientationEvent === undefined) {
            obj.isSupported = false;
            obj.error = '设备不支持方向与位置获取功能';
            resolve(obj);
        }
        else if (!w.DeviceOrientationEvent.requestPermission) {
            var unlocked_1 = false;
            var callFunc_1 = function () {
                unlocked_1 = true;
            };
            w.addEventListener('deviceorientation', callFunc_1, false);
            vf.gui.Scheduler.setTimeout(3000, function () {
                if (!unlocked_1) {
                    obj.isSupported = false;
                    obj.error = '获取方向与动作失败，检查设备是否开启选项！';
                }
                else {
                    obj.isSupported = true;
                }
                w.removeEventListener('deviceorientation', callFunc_1);
                resolve(obj);
            });
        }
    });
...
function network() {
    ...
    return new Promise(function (resolve, reject) {
        var url = '//s.vipkidstatic.com/fe-static/tvcore/detectsp_v4.js';
        var file = url + "?t=" + +new Date();
            ...
            xmlhttp_1.onreadystatechange = function () {
                if (xmlhttp_1.readyState == 4) {
                    if (xmlhttp_1.status == 200) {
                        var filesize = xmlhttp_1.responseText.length; // bytes
                        endMS = +new Date();
                        var duration = endMS - startMS;
                        var speed = (filesize / duration).toFixed(2) + "k/s";
                        obj.speed = speed;
                    }
                    else {
                        obj.error = '网络测速失败！';
 ...
 * 获取设备信息
 */
function systemInfo(callFunc) {
    var step = 0; //当前步数
    var total = 5; //总步数
    var info = {}; //设备信息
    //设备信息
    info.systemInfo = vf.utils.getSystemInfo();
    //webgl
    info.systemInfo.isWebGLSupported = vf.utils.isWebGLSupported();
    //音频播放
    info.audio = audio();
    //视频播放
    info.video = video();
    //麦克风
    microphone().then(function (value) {
        info.microphone = value;
        step++;
        callFunc(info, 'microphone', step, total);
    });
    //摄像头
    camera().then(function (value) {
        info.camera = value;
        step++;
        callFunc(info, 'camera', step, total);
    });
    //陀螺仪
    deviceOrientation().then(function (value) {
        info.deviceOrientation = value;
        step++;
        callFunc(info, 'deviceOrientation', step, total);
    });
    //网络状态
    network().then(function (value) {
        info.network = value;
        step++;
        callFunc(info, 'network', step, total);
    });
    step++;
    callFunc(info, 'baseInfo', step, total);
}
